...
#include <stdlib.h>
#include <iostream>
#include "/home/spencer/libdragon/NEW/libdragon/include/display.h"
#include "/home/spencer/libdragon/NEW/libdragon/include/graphics.h"
#include "/home/spencer/libdragon/NEW/libdragon/include/font.h"
...
namespace Graphics 
{
    ...
    uint32_t GetPixel( display_context_t disp, int x, int y )
    {
        if( disp == 0 ) { return 0; }
    ...
    uint32_t SetPixel( display_context_t disp, int x, int y, int color) {

        if( !disp ) return 0;

        uint32_t *buffer = (uint32_t *)__get_buffer( disp );
        __set_pixel( buffer, x, y, color);
    }

    uint32_t MakeColor( int r, int g, int b, int a )
    {
        color_t color;
    ...
    uint32_t ConvertColor( color_t color )
    {
        if( __bitdepth == 2 )
        {
            // 8 to 5 bit;
    ...
    void SetColor( uint32_t forecolor, uint32_t backcolor )
    {
        f_color = forecolor;
        b_color = backcolor;
    }
    ...
    void DrawPixel( display_context_t disp, int x, int y, uint32_t color )
    {
        if( disp == 0 ) { return; }
    ...
    void DrawTextFormat(display_context_t disp, int x, int y, const char* format, ...) {
		va_list args;
    ...
    void DrawPixel_trans( display_context_t disp, int x, int y, uint32_t color )
    {
        if( disp == 0 ) { return; }
    ...
    void DrawLine( display_context_t disp, int x0, int y0, int x1, int y1, uint32_t color )
    {
        int dy = y1 - y0;
        int dx = x1 - x0;
        int sx, sy;
    ...
    void DrawLine_trans( display_context_t disp, int x0, int y0, int x1, int y1, uint32_t color )
    {
        int dy = y1 - y0;
        int dx = x1 - x0;
        int sx, sy;
        ...
        DrawPixel_trans(disp, x0, y0, color);
        if(dx > dy)
        {
            int frac = dy - (dx >> 1);
            while(x0 != x1)
    ...
    void DrawBox( display_context_t disp, int x, int y, int width, int height, uint32_t color )
    {
        if( disp == 0 ) { return; }
    ...
    void DrawBox_trans( display_context_t disp, int x, int y, int width, int height, uint32_t color )
    {
        if( disp == 0 ) { return; }
    ...
    void FillScreen( display_context_t disp, uint32_t c )
    {
        if( disp == 0 ) { return; }
    ...
    void DrawCharacter( display_context_t disp, int x, int y, char ch )
    {
        if( disp == 0 ) { return; }
    ...
    void DrawText( display_context_t disp, int x, int y, const char * const msg )
    {
        if( disp == 0 ) { return; }
        if( msg == 0 ) { return; }
    ...
    void DrawSprite( display_context_t disp, int x, int y, sprite_t *sprite )
    {
        /* Simply a wrapper to call the original functionality */
        DrawSpriteStride( disp, x, y, sprite, -1 );
    }

    void DrawSpriteStride( display_context_t disp, int x, int y, sprite_t *sprite, int offset )
    {
        /* Sanity checking */
        if( disp == 0 ) { return; }
        if( sprite == 0 ) { return; }
    ...
    void DrawFont( display_context_t disp, int x, int y, int color, sprite_t *sprite, int offset )
    {
        /* Sanity checking */
        if( disp == 0 ) { return; }
        if( sprite == 0 ) { return; }
        ...
        else if( __bitdepth == 4 && sprite->bitdepth == 4 )
        {
            ...
            for( int yp = sy; yp < ey; yp++ )
            {
                ...
                for( int xp = sx; xp < ex; xp++ )
                {
                    ...
                    /* Get new color */
                    uint32_t sr = color >> 24;
                    uint32_t sg = color >> 16;
                    uint32_t sb = color >> 8;
    ...
    void DrawSpriteTrans( display_context_t disp, int x, int y, sprite_t *sprite )
    {
        /* Simply a wrapper to call the original functionality */
        DrawSpriteTransStride( disp, x, y, sprite, -1 );
    }

    void DrawSpriteTransStride( display_context_t disp, int x, int y, sprite_t *sprite, int offset )
    {
        /* Sanity checking */
        if( disp == 0 ) { return; }
        if( sprite == 0 ) { return; }
